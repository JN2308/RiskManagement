...
module.exports = cds.service.impl(async function () {
    ...
    this.on("READ", Risks, async (req, next) => {
        ...
        const busienssPartners = await BPsrv.transaction(req).send({ query: SELECT.from(this.entities.BusinessPartners).where({ BusinessPartner: bpIDs }), headers: { apikey: process.env.apikey, } });
        const bpMap = {};
        for (const businessPartner of busienssPartners) bpMap[businessPartner.BusinessPartner] = businessPartner;
        for (const note of asArray(risks)) { note.bp = bpMap[note.bp_BusinessPartner]; }
        return risks;
    })
});